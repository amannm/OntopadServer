...
public class OntologyServlet extends HttpServlet {
    ...
    private static final Logger log = Logger.getLogger(OntologyServlet.class.getName());

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        Identity id = (Identity) session.getAttribute(Attributes.IDENTITY_ATTRIBUTE);
        log.log(Level.INFO, "Ontology browsing identity: {0}", id.getNominalGroupID());

        response.setContentType("text/html");
        response.setCharacterEncoding("UTF-8");
        try (PrintWriter out = response.getWriter()) {
            HTMLService.printPlainHeader(out, "Ontopad - Ontology");
            HTMLService.printMenuBar(out, id);
            out.print("<p>This is an ontology</p>");
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        Identity id = (Identity) session.getAttribute(Attributes.IDENTITY_ATTRIBUTE);
        log.log(Level.INFO, "PUT Ontology request from identity: {0}", id.getNominalGroupID());
