...
public class IdentityData implements Serializable {
    ...
    public boolean put(String key, Identity value) {
        DatabaseEntry keyEntry = new DatabaseEntry();
        StringBinding.stringToEntry(key, keyEntry);

        DatabaseEntry valueEntry = new DatabaseEntry();
        EntryBinding valueBinding = new SerialBinding(classCatalog, Identity.class);
    ...
    public Identity get(String key) {
        ...
        if (values.get(null, keyEntry, valueEntry, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
            EntryBinding valueBinding = new SerialBinding(classCatalog, Identity.class);
            return (Identity) valueBinding.entryToObject(valueEntry);
        }
        return null;
    }
